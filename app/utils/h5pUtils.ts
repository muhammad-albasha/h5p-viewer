...
const determineH5PType = (contentPath: string): string => {
  try {
    ...
    if (dirs.some((dir) => dir.includes("Questionnaire")))
      return "Questionnaire";
    if (dirs.some((dir) => dir.includes("InteractiveVideo")))
      return "Interactive Video";
    if (dirs.some((dir) => dir.includes("Course")))
...
const assignTags = (type: string, name: string): string[] => {
  ...
  if (
    ...
    name.toLowerCase().includes("grammatik")
  )
    tags.push("Grammatik");
  if (
    name.toLowerCase().includes("vocabulary") ||
    name.toLowerCase().includes("vokabeln")
  )
    tags.push("Wortschatz");
...
export async function getH5PContents(): Promise<H5PContent[]> {
  try {
    ...
    if (dbContents.length > 0) {
      return dbContents.map((content) => ({
        ...
        tags:
          ...
          assignTags(content.contentType || "Unknown", content.title),
        slug: content.slug,
    ...
    const contentDirs = fs
      .readdirSync(h5pDir, { withFileTypes: true })
      .filter((dirent) => dirent.isDirectory())
      .map((dirent) => dirent.name);

...
function PlayH5p({ h5pJsonPath }: PlayH5pProps) {
  ...
  useEffect(() => {
    ...
    <div className={`${styles.h5pStyleWrapper} w-full bg-white/80 backdrop-blur-sm rounded-xl overflow-hidden border border-white/20 shadow-lg`}>      {loading && (
        <div className="flex flex-col items-center justify-center min-h-[300px] space-y-8 p-12 bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50 rounded-xl">
          ...
          <div className="relative">
            ...
            <div className={`absolute top-2 left-2 w-16 h-16 border-4 border-purple-200 rounded-full animate-spin ${styles.spinnerMiddle}`}>
            ...
            {/* Inner rotating ring */}
            <div className={`absolute top-4 left-4 w-12 h-12 border-4 border-pink-200 rounded-full animate-spin ${styles.spinnerInner}`}>
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-gray-600">Interaktives Lernmaterial wird vorbereitet...</p>
            
            {/* Progress dots */}
            <div className="flex items-center justify-center space-x-1 mt-4">
              <div className="w-2 h-2 bg-blue-500 rounded-full animate-bounce"></div>
              <div className={`w-2 h-2 bg-purple-500 rounded-full animate-bounce ${styles.bounceDelay1}`}></div>
              <div className={`w-2 h-2 bg-pink-500 rounded-full animate-bounce ${styles.bounceDelay2}`}></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-xl border border-white/20 p-6">
          <div className="flex gap-4 items-start">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-lg mb-1 text-red-600">Fehler beim Laden des H5P-Inhalts</h3>
        ...
        ref={h5pContainer} 
        className={`h5p-container w-full relative z-10 ${error || loading ? styles.h5pContainerHidden : styles.h5pContainer}`}
      ></div>
    </div>
  );
}

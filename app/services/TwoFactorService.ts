...
export class TwoFactorService {
   ...
   */
  generateSecret(username: string, email: string): { secret: string; qrCodeUrl: string; backupCodes: string[] } {
  ...
  async verify2FAForUser(userId: number, token: string): Promise<boolean> {
    try {
      console.log('TwoFactorService: Verifying 2FA for user', userId, 'with token', token);

...
export default function AdminDashboard() {
  ...
  const toggleFeatured = async (contentId: number) => {
    try {
      ...
      setContents((prev) =>
        prev.map((content) =>
          content.id === contentId ? { ...content, isToggling: true } : content
        )
      );
      ...
      setContents((prev) =>
        prev.map((content) =>
          content.id === contentId ? { ...content, isToggling: false } : content
        )
      );
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            {isLoading ? (
              ...
              <div className="p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  Keine H5P Inhalte gefunden
                </h3>
                <p className="text-gray-600 mb-6">
                  Erstellen Sie Ihren ersten interaktiven Lerninhalt
                </p>
                <Link
                  href="/admin/upload"
                  className="inline-flex items-center px-6 py-3 bg-primary text-white font-semibold rounded-xl hover: transition-colors duration-200"
                >
                  <svg

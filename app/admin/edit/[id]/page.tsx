...
export default function EditContent() {
  ...
  }, [status, router]);  // Fetch content data when component loads
  useEffect(() => {
    const fetchContentData = async () => {
      try {
        ...
        setSubjectAreas(Array.isArray(data.subjectAreas) ? data.subjectAreas : []);
    ...
    try {
      ...
      if (content) {
        setContent({
          ...
          title,
          subject_area_id: subjectAreaId,
          subject_area_name: subjectAreas.find(sa => sa.id === subjectAreaId)?.name || null,
          ...
          {error ? (
            <div className="alert alert-error shadow-lg mb-8">
              ...
              <div className="flex-none">
                <Link href="/admin" className="btn btn-sm">Zurück zum Dashboard</Link>
                <button onClick={() => window.location.reload()} className="btn btn-sm ml-2">Erneut versuchen</button>
              </div>
            </div>
          ) : content ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* Left Column - Edit Form */}
              <div className={`${previewMode ? 'hidden lg:block' : ''} lg:col-span-1`}>
                ...
                <div className="mt-8 bg-base-100 rounded-xl shadow-xl overflow-hidden">
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="text-sm font-semibold block mb-1">ID:</span>
                        <span className="badge">{content.id}</span>
                      </div>
                      <div>
                        <span className="text-sm font-semibold block mb-1">Typ:</span>
                        <span className="badge badge-accent">{content.content_type || "Unbekannt"}</span>
                      </div>
                      <div>
                        <span className="text-sm font-semibold block mb-1">Erstelldatum:</span>
                        <span>{new Date(content.created_at).toLocaleDateString()}</span>
                      </div>
                      <div>
                        <span className="text-sm font-semibold block mb-1">Slug:</span>
                        <span className="text-sm font-mono bg-base-200 px-2 py-1 rounded">{content.slug}</span>
                      </div>
                    </div>
              ...
              <div className={`${!previewMode && !isLoading ? 'hidden lg:block' : ''} lg:col-span-2`}>
                <div className="bg-base-100 rounded-xl shadow-xl overflow-hidden">
                  ...
                  <div className="p-6">
                    {content && content.file_path && (
                      <div className="rounded-lg overflow-hidden border border-base-300">
                        <PlayH5p h5pJsonPath={`/h5p/${content.slug}/h5p.json`} />
                      </div>
                    )}
                  </div>
                </div>

...
export async function GET(request: NextRequest) {
  try {
    ...
    const isEnabled = await twoFactorService.is2FAEnabled(
    ...
    switch (action) {
      case "generate":
        ...
        const {
          ...
          qrCodeUrl,
          backupCodes,
        } = twoFactorService.generateSecret(user.username, user.email);
      ...
      case "verify":
        ...
        const isValid = await twoFactorService.verify2FAForUser(
          parseInt(session.user.id),
          token
        );
